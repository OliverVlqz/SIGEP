<%-- 
    Document   : PublicacionesUsuario
    Created on : 22 ago. 2023, 23:28:52
    Author     : olive
--%>
...
Usuario usuarioParaPublicaciones;

String usuarioParaPublicacionesId = (String) session.getAttribute("usuarioParaPublicacionesId");
if (usuarioParaPublicacionesId != null) {
    Long usuarioId = Long.parseLong(usuarioParaPublicacionesId);
    usuarioParaPublicaciones = control.findUsuarioById(usuarioId);  // Asumiendo que tienes un método para obtener un usuario por ID en tu clase controladora
} else {
    usuarioParaPublicaciones = usuarioLogueado;
}

List<Publicacion> publicacionesUsuario = listaPublicaciones.stream()
    .filter(publi -> publi.getUsuario().getId().equals(usuarioParaPublicaciones.getId()))
    .collect(Collectors.toList());


// Si el usuario no tiene publicaciones, redirige a otro JSP
if (publicacionesUsuario.isEmpty()) {
    response.sendRedirect("adminVerUsuario.jsp");
    return;
}

for(Publicacion publi : publicacionesUsuario) { 
